...
printPoolContent (BM_BufferPool *const bm)
{
	...
	printf("{");
	printStrat(bm);
	printf(" %i}: ", bm->numberPgs);

	for (i = 0; i < bm->numberPgs; i++)
...
sprintPoolContent (BM_BufferPool *const bm)
{
	...
	message = (char *) malloc(256 + (22 * bm->numberPgs));
	frameContent = getFrameContents(bm);
	dirty = getDirtyFlags(bm);
	fixCount = getFixCounts(bm);
...
void
printpgcontent (BM_PageHandle *const page)
{
	int i;
...
char *
sprintpgcontent (BM_PageHandle *const page)
{
	int i;
	char *message;
	int pos = 0;
